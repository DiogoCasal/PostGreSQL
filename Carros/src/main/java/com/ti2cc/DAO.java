...
public class DAO {
	...
	public boolean inserirCarro(Carro carro) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO carro (codigo, nome, modelo, placa) "
	...
	public boolean atualizarCarro(Carro carro) {
		...
		try {  
			Statement st = conexao.createStatement();
			String sql = "UPDATE carro SET nome = '" + carro.getNome() + "', modelo = '"  
				       + carro.getmodelo() + "', placa = '" + carro.getplaca() + "'"
	         ...
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                carros[i] = new Carro(rs.getInt("codigo"), rs.getString("nome"), 
	                		                  rs.getString("modelo"), rs.getInt("placa"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
